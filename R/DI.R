...
#' This allows users to provide already simultaneous trajectories to speed up processing. If sim = T, 
#' traj1 and traj2 may be provided as dataframes. 
#' @return
...
DI <- function(traj1, traj2, tc = 0, local = FALSE, rand = 99, alpha = 1, sim = F) {
  if (!sim) {
  ...
  if (!exists("tr2")) {
    tr2 <- if(class(traj2)[1] == "ltraj"){ld(traj2)}else{traj2}   
  }
  
  pkey1 <- as.character(tr1$pkey[1])
  pkey2 <- as.character(tr2$pkey[2])
